...
def reverse_readline(filename, buf_size=1048576):
    ...
    with open(filename, 'rb') as fh:
        ...
        while remaining_size > 0:
            ...
            for index in range(len(lines) - 1, 0, -1):
                if lines[index]:
                    yield lines[index].decode('utf-8', errors='ignore')
        
        if segment is not None:
            yield segment.decode('utf-8', errors='ignore')

def main():
    print("Pass 1: Finding 120 newer valid arxiv papers (reading backwards)...")
    selected_papers = []
    reference_ids = set()
    
    # Keep track of existing PDFs to avoid duplicates
    existing_pdfs = set(os.listdir(PDF_DIR))
    
    for line in reverse_readline(DATASET_PATH):
        line = line.strip()
        if line.startswith(','):
            line = line[1:]
        ...
        if 'arxiv' in line.lower():
            try:
                ...
                if arxiv_id:
                    ...
                    clean_t = "".join(c if c.isalnum() else '_' for c in paper.get('title', ''))[:60].strip('_')
                    pdf_filename = f"{paper.get('year', '0000')}_{clean_t}.pdf"
                    ...
                    if is_valid_pdf(arxiv_id):
                        ...
                        print(f"Found newer valid {len(selected_papers)}: {arxiv_id} (Year: {paper.get('year')}, Refs: {len(refs)})")
    ...
    print("Pass 2: Extracting reference metadata from dataset (reading forwards)...")
    resolved_references = {}
        ...
        for i, line in enumerate(f):
            ...
            if match:
                ...
                if pid in reference_ids:
                    try:
                        resolved_references[pid] = json.loads(line)
